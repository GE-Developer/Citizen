...
extension DictionaryView {
    ...
    private func wordCard(_ word: SavedWord) -> some View {
        ...
        // Форма превью контекстного меню = скругление карточки (иначе у меню жёсткие углы).
        .contentShape(.contextMenuPreview, RoundedRectangle(cornerRadius: 15))
        .contextMenu { removeButton(word) }
        // Фаза 1: ячейка тает на своём месте (зазор пока сохраняется).
        .opacity(fadingKeys.contains(word.key) ? 0 : 1)
        .padding(.bottom, 16) // отступ карточки — уезжает вместе с ней при удалении
        // К моменту удаления ячейка уже невидима → без повторного fade и наложения.
        .transition(.identity)
    }
    ...
    private func lemmaSection(_ word: SavedWord) -> some View {
            VStack(alignment: .leading, spacing: 2) {
                Text(word.lemmaWord)
                    ...
                    .fontDesign(.rounded)
                    .foregroundStyle(Color.citizen.mainText)
                Text("[\(word.lemmaTransliteration)]")
                    .font(.subheadline)
